...
const c = document.getElementById('createdBy');
// se puede usar tambien: const c = document.querySelector('#createdBy');
c.innerHTML = 'Aplicación creada por Diego'
// tb puede ser c.innerHTML = c.innerHTM + ' ' + 'Diego'
...
function buildToDos(toDos) {
  // Tu código acá:
  const arrBuilt = toDos.map(function(element, index){
    return buildToDo(element, index); // return para que retorne y se guarde en arrBuilt 
  });
  // const arrBuilt = toDos.map(buildToDo) tambien se puede
  return arrBuilt
  };
...
function displayToDos() {
  ...
  let rdoBuild = buildToDos(toDoItems);
  rdoBuild.map(function(element){
    toDoContainer.appendChild(element); 
  })
  // o for loop:
  // for (let i = 0; i<rdoBuild.length; i++){
  //   toDoContainer.appendChild(rdoBuild[i]);
  // }
}
...
// addToDo sin parentesis para que no se ejecute esa funcion, sino cuando le den click, no cuando se declare en el codigo:

// de otra forma
// let ids = document.querySelector('#addButton');
// ids.addEventListener('click', addToDo);
...
function completeToDo(event) {
  ...
  const index = event.target.id;  // para sacar el id del evento
  // Tu código acá:
  toDoItems[index].completeToDo(); //entra al objeto que se le hizo click y lo completa
  displayToDos()
